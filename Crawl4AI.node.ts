...
class Crawl4aiCustom implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				const outputFormats = this.getNodeParameter('outputFormat', i) as string[];
				...
				const crawlerOptions = this.getNodeParameter('crawlerOptions', i) as any;
				...
				if (crawlerOptions.excludedTags) {
					crawlerConfig.excluded_tags = crawlerOptions.excludedTags.split(',').map((tag: string) => tag.trim());
				}
				if (crawlerOptions.excludedClasses) {
					crawlerConfig.excluded_classes = crawlerOptions.excludedClasses.split(',').map((cls: string) => cls.trim());
				}
				if (crawlerOptions.excludedIds) {
					crawlerConfig.excluded_ids = crawlerOptions.excludedIds.split(',').map((id: string) => id.trim());
				}
				...
				const extractionStrategy = this.getNodeParameter('extractionStrategy', i) as string;
				let extraction: any = null;

				if (extractionStrategy !== 'none') {
					const rawExtractionStrategy = this.getNodeParameter('rawExtractionStrategy', i, '{}') as string;
					...
					switch (extractionStrategy) {
						case 'css':
							...
							const cssSelectors = this.getNodeParameter('cssSelectors', i) as any;
						...
						case 'xpath':
							const xpathSelectors = this.getNodeParameter('xpathSelectors', i) as any;
							extraction = {
								type: 'xpath',
						...
						case 'llm':
							const llmOptions = this.getNodeParameter('llmOptions', i) as any;
							extraction = {
								type: 'llm',
						...
						case 'custom':
							const customExtractionStrategy = this.getNodeParameter('customExtractionStrategy', i) as string;
				...
				if (this.continueOnFail()) {
					...
					try {
						...
						errorUrl = op === 'crawl' ? (this.getNodeParameter('url', i) as string) : 'Multiple URLs';
					} catch (e) {
